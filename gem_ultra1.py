...
class UltraGeMScraper:
    def __init__(self, username, start_idx, end_idx, run_id, log_callback=None): #runid compulsion
        ...
        self.OUTPUT_BASE = os.path.join(self.BACKEND_DIR, "outputs", "gem", self.run_id)

        # Legacy-compatible sub-directories
        self.SCRAPE_DIR = os.path.join(self.OUTPUT_BASE, "gemscrape")
        self.CSV_OUTPUT_DIR = os.path.join(self.OUTPUT_BASE, "Gem_CSVs")
        self.REPORT_DIR = os.path.join(self.OUTPUT_BASE, "Scraping_Reports")
        self.DUPS_DIR = os.path.join(self.OUTPUT_BASE, "Gem_Duplicates")

        for folder in [self.OUTPUT_BASE, self.SCRAPE_DIR, self.CSV_OUTPUT_DIR, self.REPORT_DIR, self.DUPS_DIR]:
            os.makedirs(folder, exist_ok=True)
    ...
    async def scrape_state(self, state, today, tomorrow, semaphore, index):
        async with semaphore:
            ...
            state_dir = os.path.join(self.SCRAPE_DIR, state)
            ...
            if state_bids:
                ...
                if not df_duplicates.empty:
                    ...
                    dups_path = os.path.join(self.DUPS_DIR, f"{state}_duplicates.csv")
                ...
                csv_path = os.path.join(self.CSV_OUTPUT_DIR, f"{state}.csv")
    ...
    def _write_master_excel(self):
        ...
        for state in self.target_states:
            ...
            excel_path = os.path.join(
                self.SCRAPE_DIR, state, f"gem_output_of_{state_safe}.xlsx"
            )
        ...
        if all_dfs:
            ...
            excel_out = os.path.join(self.OUTPUT_BASE, f"gem_output_of_merged_{self.run_id}.xlsx")
            ...
            csv_out = os.path.join(self.OUTPUT_BASE, f"gem_merged_data_{self.run_id}.csv")
    ...
    def generate_report(self, duration):
        ...
        report_path = os.path.join(self.REPORT_DIR, f"ultra_report_{date_str}.txt")
        ...
        self._log(f"Output directory: {self.OUTPUT_BASE}")
